...
import { GridActionEnum, GridFunctionType } from "../types";
...
export function useUndoRedo(
  ...
  gridFunction: GridFunctionType | null,
  isEdittable: boolean,
  gridData?: any[],
  ...
  useEffect(() => {
    if (selectedRow !== undefined && selectedRow.length > 0 && gridData) {
      let rowSelection = CompactSelection.empty();
      selectedRow.forEach((selectedItem) => {
        const index = gridData.findIndex((item) => {
          const keys = Object.keys(selectedItem);
          return item[keys[1]] === selectedItem[keys[1]];
        });
        if (index !== -1) {
          rowSelection = rowSelection.add([index, index + 1]);
        }
      });
      ...
      gridSelectionRef.current = initialGridSelection;
    } else {
  ...
  const onGridSelectionChangedEdited = useCallback(
    (newVal: GridSelection) => {
      ...
      if (
        newVal.current === undefined &&
        newVal.columns.toArray().length === 0 &&
        newVal.rows &&
        gridData &&
        gridData?.length > 0
      ) {
        setGridSelection(newVal);
        const rows = newVal.rows.toArray();
        const selectedDataRows: any[] = [];
        rows.map((row) => {
          const dataRow = gridData[row];
          selectedDataRows.push(dataRow);
        });
        gridFunction &&
          gridFunction(GridActionEnum.RowSelect, selectedDataRows);
      }
    },
    [onGridSelectionChange, gridData]
  );
