...
function onClickSelect(
  e: Parameters<NonNullable<CustomRenderer<LinksCell>["onSelect"]>>[0]
) {
  ...
  for (const [index, l] of links.entries()) {
    ...
    const commaMetrics = needsComma
      ? measureTextCached(l.title + ",", ctx, font)
      : metrics;
...
const renderer: CustomRenderer<LinksCell> = {
  ...
  draw: (args, cell) => {
    ...
    for (const [index, l] of links.entries()) {
      ...
      const isHovered =
        rectHoverX > drawX && rectHoverX < drawX + metrics.width;
      ...
      if (isHovered) {
        ...
        ctx.lineTo(
          drawX + metrics.width,
          Math.floor(drawY + underlineOffset) + 0.5
        );
...
const LinksCellEditorStyle = styled(Box)(() => ({
  display: "flex",
  ...
  "& > button": {
    ...
    cursor: "pointer",
    "&:hover, &:focus-visible": {
      backgroundColor: "var(--gdg-accent-light)",
    },
    "&:disabled": {
      opacity: 0.4,
      pointerEvents: "none",
