...
const renderer: CustomRenderer<StarCell> = {
  ...
  isMatch: (cell: CustomCell): cell is StarCell =>
    (cell.data as any).kind === "star-cell",
  needsHover: true,
  draw: (args, cell) => {
      ...
      >
        {[0, 1, 2, 3, 4].map((index) => (
          <div
            key={index}
            // className={
            //   p.value.data.rating < index + 1 ? "gdg-inactive" : "gdg-active"
            // }
            onClick={() => {
              args.data.onStarChange(index + 1);
              p.onChange({
                ...p.value,
                data: {
                  ...p.value.data,
          ...
          >
            <Star
              sx={{ color: p.value.data.rating > index ? "black" : "grey" }}
            />
          </div>
        ))}
      </Box>
