...
export const getSquareXPosFromAlign = (
  ...
  switch (alignment) {
    ...
    case "right":
      return (
        Math.floor(containerX + containerWidth) -
        horizontalPadding -
        squareWidth / 2
      );
  }
...
export function useStateWithReactiveInput<T>(
  ...
  const inputStateRef = React.useRef<[T | typeof empty, T]>([
    empty,
    inputState,
  ]);
...
export function makeAccessibilityStringForArray(
  arr: readonly string[]
): string {
