...
import { ToolBoxModelType } from "@/app/canvas/[slug]/modelCreator/_lib/_store/modelStore/ToolBoxModelStore";
...
export const transformToolBoxModelsToTreeData = (
  models: ToolBoxModelType[],
  expandedNodes?: Record<string, boolean>
): TreeDataType[] => {
  const findModelById = (
    models: ToolBoxModelType[],
    modelId: string
  ): ToolBoxModelType | undefined =>
  ...
  const transformAttributeToNode = (
    ...
    if (
      ...
      const referencedModel = findModelById(models, attr.dataSourceId);
      if (referencedModel) {
        const childrenNodes: TreeDataType[] =
          referencedModel.config.attributes.map((attribute) =>
            transformAttributeToNode(attribute, attr.id, depth + 1, maxDepth)
          );
        node.children = childrenNodes;
      }
  ...
  return models.map((model) => {
    const filteredAttributes = model.config.attributes.filter(
      (attribute) => !attribute.key
    );
    return {
      id: model.id,
      parentId: null,
      parentName: null,
      title: model.friendlyName || model.name,
      name: model.name,
      dataType: "model",
      draggable: true,
      dataSourceId: model.id,
      isOpen: expandedNodes ? expandedNodes[model.id] : false,
      dropType: DropTypes.Model,
      children:
        model.config.attributes.length > 0
          ? filteredAttributes.map((attr) =>
              transformAttributeToNode(attr, model.id, 0, 10)
            )
          : [],
    };
  });
